use cap_scale::presets::{TokenPreset, AspectMode, Size, build_plan, ScalePlan};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("🔧 Using actual scaling functions from cap_scale crate");
    println!();
    ...
    println!("📏 Test Image: {}x{} ({} pixels)", test_width, test_height, test_width * test_height);
    ...
    // Control: no scaling
    let control_size = Size { w: test_width, h: test_height };
    let control_tokens = calculate_vision_tokens(control_size);
    println!("🎯 Control (no scaling): {}x{} = {} tokens", control_size.w, control_size.h, control_tokens);
    ...
    for (preset, name) in &presets {
        ...
        let plan = build_plan(
            Size { w: test_width, h: test_height },
            preset.to_target(),
            AspectMode::Preserve
        );
        ...
        let actual_output_size = Size { w: scaled_result.width, h: scaled_result.height };
    ...
    let control_4k_size = Size { w: test_4k_width, h: test_4k_height };
    let control_4k_tokens = calculate_vision_tokens(control_4k_size);
    println!("🎯 Control (no scaling): {}x{} = {} tokens", control_4k_size.w, control_4k_size.h, control_4k_tokens);
    ...
    for (preset, name) in &presets {
        ...
        let plan = build_plan(
            Size { w: test_4k_width, h: test_4k_height },
            preset.to_target(),
            AspectMode::Preserve
        );
    ...
    for (name, width, height) in aspect_ratios {
        ...
        let control_size = Size { w: width, h: height };
        let control_tokens = calculate_vision_tokens(control_size);
        println!("  Control: {}x{} = {} tokens", control_size.w, control_size.h, control_tokens);
...
/// Apply scaling using the actual cap_scale functions
fn apply_scaling(input_bgra: &[u8], width: u32, height: u32, plan: &ScalePlan) -> Result<ScaledImage, Box<dyn std::error::Error>> {
