...
use cap_scale::presets::{build_plan, AspectMode, Size, TokenPreset};
...
fn main() -> anyhow::Result<()> {
    ...
    let plan = build_plan(
        Size { w: src_w, h: src_h },
        TokenPreset::P9_Long640.to_target(),
        AspectMode::Preserve,
    );
    ...
    let mut tiles: Vec<Vec<u8>> = (0..4)
        .map(|_| vec![0u8; (cfg.tile_side as usize) * (cfg.tile_side as usize) * 4])
        .collect();
    let tiles_slices: Vec<&mut [u8]> = tiles.iter_mut().map(|v| v.as_mut_slice()).collect();
    let mut global = vec![0u8; (cfg.global_side as usize) * (cfg.global_side as usize) * 4];
    let outs = GundamOutputs {
        tiles: tiles_slices,
